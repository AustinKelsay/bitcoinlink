...
const LinkModal: React.FC<LinkModalProps> = ({
  ...
  useEffect(() => {
    if (generatedLinks && generatedLinks.length > 0) {
      ...
      const valueBetween = generatedLinks[0].substring(
        lastSlashIndex + 1,
        questionMarkIndex
      );
      ...
      <TabView>
              ...
              {generatedLinks?.map((link, index) => (
                ...
                >
                  ...
                  <Button
                    ...
                    severity="success"
                    aria-label="copy"
                    onClick={() => copyToClipboard(`https://www.${link}`)}
                  />
                </div>
              ))}
            </div>
        ...
        {generatedLinks && generatedLinks.length > 0 && nwcId && (
          <TabPanel header="API Integration">
            <div className="p-4 bg-gray-800 text-white">
              <p className="mb-4">
                To generate links programmatically, use the API endpoint below
                with the provided secret. Please note that the secret is only
                available on this page and will not be shown again. If you leave
                this page, you will need to generate new links to obtain a new
                secret.
              </p>
              <div className="bg-gray-700 p-4 rounded-md shadow-md">
                ...
                <div className="overflow-x-auto mb-4">
                  <pre className="bg-gray-900 text-white p-2 rounded-md">
                    <code className="bg-gray-900 text-white p-2 rounded-md">
                      GET https://www.bitcoinlink.app/api/link/{oneToManyNwcId}
                    </code>
                  </pre>
                </div>
                ...
                <p>
                  To get a link, make a GET request to the API URL with the
                  secret included in the Authorization header.
                </p>
              </div>
            </div>
          </TabPanel>
